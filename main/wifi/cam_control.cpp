...
void runcam_protocol_listen_task(void* param) {
    ...
    ESP_ERROR_CHECK(uart_driver_install(UART_NUM_0, kBufSize * 2, 0, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_param_config(UART_NUM_0, &uart_config));
    ESP_ERROR_CHECK(
        uart_set_pin(UART_NUM_0, UART_PIN_NO_CHANGE, rx_gpio, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));

    uint8_t buf[4];
    int pos = 0;
    while (1) {
        uart_read_bytes(UART_NUM_0, buf + pos, 1, portMAX_DELAY);
...
void lanc_listen_task(void* param) {
    ...
    ESP_ERROR_CHECK(uart_driver_install(UART_NUM_0, kBufSize * 2, 0, 0, NULL, ESP_INTR_FLAG_IRAM));
    ESP_ERROR_CHECK(uart_param_config(UART_NUM_0, &uart_config));
    ESP_ERROR_CHECK(
        uart_set_pin(UART_NUM_0, UART_PIN_NO_CHANGE, rx_gpio, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));
