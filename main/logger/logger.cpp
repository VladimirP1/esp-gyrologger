...
static void logger_task_cpp(void *params_pvoid) {
    ...
    for (int i = 0;; ++i) {
        ...
        if (logger_active) {
            ...
            if (i % 1000 == 0) {
                // auto r = quat::to_aa(q);
                ESP_LOGI("gyro_consumer", "i=%d, ts = %lld, qZ = %f, max_backlog = %d", i,
                         msg.timestamp, q[3], max_backlog);
            }
            q = {1,0,0,0};
        ...
        if (quat_encoder.has_block(kBlockSize)) {
            ...
            while (buf_ptr != buf_end_ptr) {
                int written = fwrite(
                    buf_ptr, 1,
                    std::min(static_cast<size_t>(16), static_cast<size_t>(buf_end_ptr - buf_ptr)),
                    f);
                buf_ptr += written;
                if (!written) {
        ...
        if (i % 1000 == 0) {
            // ESP_LOGI("gyro_consumer", "i=%d, ts = %lld, qZ = %f, max_backlog = %d", i,
            // msg.timestamp, q[3], max_backlog); ESP_LOGI("gyro_consumer", "i=%d, ts = %lld, gZ =
            // %d, max_backlog = %d", i, msg.timestamp, msg.gyro_z, max_backlog);
            max_backlog = 0;
        }
        if (i % 2000 == 0) {
            fflush(f);
