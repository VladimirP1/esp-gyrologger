...
class QuatEncoder {
   public:
    ...
    bool encode(quat::quat q) {
        ...
        while (iters--) {
            ...
            quat::vec v_update_dequant =
                quat::map(v_update_quant, [&](double x) { return deiq_i16_i8(x, gyro_scale); });
    ...
    void drop_block(size_t requested_size) {
        assert(has_block(requested_size));
        for (int i = 0; i < requested_size; ++i) bytes.pop_front();
    }
   ...
   public:
    ...
    int encode_block(I data_begin, I data_end, uint8_t *&compressed_end_ptr) {
        ...
        while (data_begin != data_end) {
            ...
            RansEncPut(&state, &compressed_ptr, start, freq, freq_scale_bits_);
