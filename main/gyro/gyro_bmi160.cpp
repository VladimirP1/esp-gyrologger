...
static void IRAM_ATTR gyro_i2c_cb(void* args) {
    ...
    proc_aux_i2c();
    mini_i2c_read_reg_callback(gctx.i2cctx, gctx.gyro_i2c_adr, is_bmi270 ? REG_FIFO_DATA_BMI270 : REG_FIFO_DATA,
                               bytes_to_read, gyro_i2c_cb, nullptr);
}

bool probe_bmi160(uint8_t dev_adr) {
    uint8_t data[1];
    if (mini_i2c_read_reg_sync(gctx.i2cctx, dev_adr, 0x00, data, 1) != ESP_OK) {
        return false;
    }
...
static bool check_is_bmi270() {
    uint8_t data[1];
    if (mini_i2c_read_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, 0x00, data, 1) != ESP_OK) {
        return false;
    }
    return data[0] == 0x24;
}
...
void gyro_bmi160_task(void* params) {
    ...
    for (int i = 0; i < 10; ++i) {
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_CMD, 0xb6);  // reset
    ...
    if (is_bmi270) {
        ...
        // disable power save
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_PWR_CONF, 0x00);
        vTaskDelay(10 / portTICK_PERIOD_MS);
        // prepare to load config file
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_INIT_CTRL, 0x00);
        // load config file
        for (int i = 0; i < sizeof(bmi270_config_file); i += 2) {
            mini_i2c_write_reg2_sync(gctx.i2cctx, gctx.gyro_i2c_adr, 0x5b, (uint8_t)((i / 2) & 0x0F),
                                     (uint8_t)((i / 2) >> 4));
            mini_i2c_write_reg2_sync(gctx.i2cctx, gctx.gyro_i2c_adr, 0x5e, bmi270_config_file[i],
                                     bmi270_config_file[i + 1]);
        }
        // complete loading config file
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_INIT_CTRL, 0x01);
        vTaskDelay(200 / portTICK_PERIOD_MS);

        mini_i2c_set_timing(gctx.i2cctx,710000);
        mini_i2c_double_stop_timing(gctx.i2cctx);
        mini_i2c_double_stop_timing(gctx.i2cctx);
        mini_i2c_double_stop_timing(gctx.i2cctx);

        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_FIFO_CONFIG_0_BMI270, 0);
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_FIFO_CONFIG_1_BMI270, 0b11010000);

        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_ACC_CONF,
                                0b10101000);  // 100hz sample rate, OSR1
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_ACC_RANGE, 3);

        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_GYR_CONF,
                                0b11101101);  // 3.2k sample rate, OSR1 (890hz LPF)
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_GYR_RANGE, 0b00001001);

        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_PWR_CONF, 0x02);
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_PWR_CTRL, 0x06);  // enable gyro + acc
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_CMD, 0xb0);       // fifo reset

        mini_i2c_read_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, 0x3c, &factor_zx, 1);

        mini_i2c_read_reg_callback(gctx.i2cctx, gctx.gyro_i2c_adr, REG_FIFO_DATA_BMI270, bytes_to_read,
                                   gyro_i2c_cb, nullptr);
    } else {
        ...
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_CMD, 0b00010001);  // start accel
        vTaskDelay(10 / portTICK_PERIOD_MS);
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_CMD, 0b00010101);  // start gyro
        vTaskDelay(100 / portTICK_PERIOD_MS);
        ...
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_ACC_RANGE, 0b1100);
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_GYR_CONF,
                                0b00101101);  // 3.2k sample rate, OSR1 (890hz LPF)
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_GYR_RANGE, 1);

        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_FIFO_CONFIG_0, 0);
        mini_i2c_write_reg_sync(gctx.i2cctx, gctx.gyro_i2c_adr, REG_FIFO_CONFIG_1, 0b11010000);

        mini_i2c_read_reg_callback(gctx.i2cctx, gctx.gyro_i2c_adr, REG_FIFO_DATA, bytes_to_read, gyro_i2c_cb,
                                   nullptr);
    }
